...
namespace VSX.UniversalVehicleCombat
{
    ...
    public class PlayerSpaceFighterControl : MonoBehaviour, IVehicleInput
    {
		...
		void SetControlValues()
		{
            ...
            {
                agent.Vehicle.Engines.SetRotationInputs(new Vector3(pitch, yaw, roll));
            }
		...
		void PowerManagementMenuControls()
		{
			...
			if (Input.GetKeyDown(KeyCode.P))
			{
                ...
                {
                    GameStateManager.Instance.EnterGameState(GameState.PowerManagementMenu);
                }
                else if (GameStateManager.Instance.CurrentGameState == GameState.PowerManagementMenu)
                {
            ...
            if (GameStateManager.Instance.CurrentGameState == GameState.PowerManagementMenu)
            {
                if (powerManagementMenuController != null)
                {
                    if (Input.GetKey(KeyCode.Keypad4))
                    {
                        powerManagementMenuController.MovePowerBall(0, -1);
                    }
                    ...
                    {
                        powerManagementMenuController.MovePowerBall(1, 0);
                    }
		...
		void TriggerGroupsMenuControls()
		{
			...
			if (Input.GetKeyDown(KeyCode.G))
			{
                if (GameStateManager.Instance.CurrentGameState == GameState.Gameplay)
                {
                    GameStateManager.Instance.EnterGameState(GameState.TriggerGroupsMenu);
                }
                else if (GameStateManager.Instance.CurrentGameState == GameState.TriggerGroupsMenu)
                {
                    GameStateManager.Instance.EnterGameState(GameState.Gameplay);
                }
            }
			...
			if (triggerGroupsMenuController != null && GameStateManager.Instance.CurrentGameState == GameState.TriggerGroupsMenu){
					
				if (Input.GetKeyDown(KeyCode.Alpha0) && GameStateManager.Instance.CurrentGameState == GameState.TriggerGroupsMenu)
				{
					triggerGroupsMenuController.SetTriggerGroupTriggerValue(0);
				}
	
				if (Input.GetKeyDown(KeyCode.Alpha1) && GameStateManager.Instance.CurrentGameState == GameState.TriggerGroupsMenu)
				{
		...
		void CameraControls()
		{
			if (!hasVehicleCamera || GameStateManager.Instance.CurrentGameState != GameState.Gameplay) return;
