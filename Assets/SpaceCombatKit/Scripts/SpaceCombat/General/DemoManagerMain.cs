...
namespace VSX.UniversalVehicleCombat
{
    ...
    /// </summary>
	public class DemoManagerMain : MonoBehaviour
	{
        ...
        [SerializeField]
        private bool attackImmediately = false;
        ...
        public void OnVehicleDestroyed(Vehicle destroyedVehicle, GameAgent attacker)
		{
			...
			{
                GameStateManager.Instance.EnterGameState(GameState.GameOver);
            }
		}
        ...
        void Update()
        {
            ...
            {
                if (rumbleManager != null) rumbleManager.AddRumble(1, 0.1f, 0f, 0.5f);
            }

            if (!attackImmediately && Input.GetKeyDown(KeyCode.Backspace) && !attackTriggered)
            {
                Ambush();
            }
        }
        ...
        /// <summary>
        /// Called to make the scene restart.
        /// </summary>
        public void Restart()
        {
		...
		IEnumerator IndividualWarp(Vehicle warpingVehicle, float warpDelay, float warpTime, Vector3 startPosition, float warpDistance)
		{
			...
			if (rumbleManager != null) rumbleManager.AddRumble(0.005f, 0.1f, 0f, 0.5f);
