...
namespace VSX.UniversalVehicleCombat
{
    ...
    public delegate void OnCockpitInteractionModeChangedEventHandler();
	...
	public class Vehicle : MonoBehaviour, ITrackable
	{
        ...
        [SerializeField]
        protected List<string> cockpitInteractionModes = new List<string>();
        public List<string> CockpitInteractionModes { get { return cockpitInteractionModes; } } 

        protected int selectedCockpitInteractionModeIndex = -1;
        public string SelectedCockpitInteractionMode
        {
            get
            {                
                return selectedCockpitInteractionModeIndex == -1 ? "" : cockpitInteractionModes[selectedCockpitInteractionModeIndex];
            }
        }
        ...
        /// <summary> The label that appears on radar for this vehicle. </summary>
        public string TrackableLabel
        {
            get
            {
                // If there is a game agent controlling this vehicle, make its name show up on radar
                if (agent != null)
                {
                    return agent.Label;
                }
                // Otherwise use the label of the vehicle itself
                else
                {
                    return label;
                }
        ...
        public OnCockpitInteractionModeChangedEventHandler onCockpitInteractionModeChangedEventHandler;
        ...
        protected virtual void Awake()
		{
            ...
            if (cockpitInteractionModes.Count > 0)
            {
                selectedCockpitInteractionModeIndex = 0;
            }
        ...
        public virtual void CycleCockpitInteractionMode (bool cycleForward)
        {

            if (cockpitInteractionModes.Count == 0) return;

            if (cycleForward)
            {
                selectedCockpitInteractionModeIndex += 1;
                if (selectedCockpitInteractionModeIndex >= cockpitInteractionModes.Count)
                {
                    selectedCockpitInteractionModeIndex = 0;
                }
            }
            ...
            {
                selectedCockpitInteractionModeIndex -= 1;
                if (selectedCockpitInteractionModeIndex == -1)
                {
                    selectedCockpitInteractionModeIndex = cockpitInteractionModes.Count - 1;
                }
            }

            if (onCockpitInteractionModeChangedEventHandler != null)
                onCockpitInteractionModeChangedEventHandler();
        }
        ...
        {
            if (updatePhysicsInfoFromRigidbody && HasRigidbody)
            {
