...
namespace VSX.UniversalVehicleCombat
{
    ...
    public class TriggerGroupsMenuController : MonoBehaviour 
	{
        ...
        {
            if (newGameState == GameState.TriggerGroupsMenu)
            {
                Activate();
            }
            else
		...
		void OnMenuChanged()
		{
            ...
            for (int i = 0; i < focusedVehicle.TriggerGroupsManager.numGroups; ++i)
			{
                ...
                {
                    if (groupItems[i].CachedGameObject.activeSelf) groupItems[i].SetGroupIndex(i);
                }
		...
		void SetMenuState(bool activate)
		{
			...
			else
			{
                OnMenuChanged();
                isActivated = true;
                if (focusedVehicle.TriggerGroupsManager.numGroups > 0 && focusedVehicle.TriggerGroupsManager.mountedTriggerables.Count > 0)
                {
                    SelectTriggerable(0, 0);
                }
			}
		}
        ...
        public void CycleGroup(bool forward)
        {
            
            if (focusedVehicle.TriggerGroupsManager.numGroups == 0) return;

            int newIndex = Mathf.Clamp(forward ? focusedGroupIndex + 1 : focusedGroupIndex - 1, 0, focusedVehicle.TriggerGroupsManager.numGroups - 1);
            
            if (newIndex != focusedGroupIndex)
            {
                SelectTriggerable(focusedTriggerableIndex, newIndex);
            }

        }


        public void CycleTriggerable(bool forward)
        {
            if (focusedVehicle.TriggerGroupsManager.mountedTriggerables.Count == 0) return;

            int newIndex = Mathf.Clamp(forward ? focusedTriggerableIndex + 1 : focusedTriggerableIndex - 1, 0, focusedVehicle.TriggerGroupsManager.mountedTriggerables.Count - 1);

            if (newIndex != focusedTriggerableIndex)
            {
                SelectTriggerable(newIndex, focusedGroupIndex);
            }
		...
		public void SelectTriggerable (int newFocusedTriggerableIndex, int newFocusedGroupIndex)
		{
