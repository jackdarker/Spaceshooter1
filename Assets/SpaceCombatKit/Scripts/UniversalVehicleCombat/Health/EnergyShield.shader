    ...
    SubShader {
        ...
        Pass { 
            ...
            v2f vert (appdata_full v) 
			{
				...
				float dist = (distance (v.vertex.xyz, _LocalHitPoint0.xyz) * _MeshScale)/(_SpreadFactor * _MeshScale);
				effectStrength = max(GetEffectValue(_LocalHitPoint0.w, dist), effectStrength), 0.0, 1.0;

				dist = (distance (v.vertex.xyz, _LocalHitPoint1.xyz) * _MeshScale)/(_SpreadFactor * _MeshScale);
				effectStrength = max(GetEffectValue(_LocalHitPoint1.w, dist), effectStrength), 0.0, 1.0;

				dist = (distance (v.vertex.xyz, _LocalHitPoint2.xyz) * _MeshScale)/(_SpreadFactor * _MeshScale);
				effectStrength = max(GetEffectValue(_LocalHitPoint2.w, dist), effectStrength), 0.0, 1.0;

				dist = (distance (v.vertex.xyz, _LocalHitPoint3.xyz) * _MeshScale)/(_SpreadFactor * _MeshScale);
				effectStrength = max(GetEffectValue(_LocalHitPoint3.w, dist), effectStrength), 0.0, 1.0;

				dist = (distance (v.vertex.xyz, _LocalHitPoint4.xyz) * _MeshScale)/(_SpreadFactor * _MeshScale);
				effectStrength = max(GetEffectValue(_LocalHitPoint4.w, dist), effectStrength), 0.0, 1.0;

				dist = (distance (v.vertex.xyz, _LocalHitPoint5.xyz) * _MeshScale)/(_SpreadFactor * _MeshScale);
				effectStrength = max(GetEffectValue(_LocalHitPoint5.w, dist), effectStrength), 0.0, 1.0;

				dist = (distance (v.vertex.xyz, _LocalHitPoint6.xyz) * _MeshScale)/(_SpreadFactor * _MeshScale);
				effectStrength = max(GetEffectValue(_LocalHitPoint6.w, dist), effectStrength), 0.0, 1.0;

				dist = (distance (v.vertex.xyz, _LocalHitPoint7.xyz) * _MeshScale)/(_SpreadFactor * _MeshScale);
				effectStrength = max(GetEffectValue(_LocalHitPoint7.w, dist), effectStrength), 0.0, 1.0;

				dist = (distance (v.vertex.xyz, _LocalHitPoint8.xyz) * _MeshScale)/(_SpreadFactor * _MeshScale);
				effectStrength = max(GetEffectValue(_LocalHitPoint8.w, dist), effectStrength), 0.0, 1.0;

				dist = (distance (v.vertex.xyz, _LocalHitPoint9.xyz) * _MeshScale)/(_SpreadFactor * _MeshScale);
