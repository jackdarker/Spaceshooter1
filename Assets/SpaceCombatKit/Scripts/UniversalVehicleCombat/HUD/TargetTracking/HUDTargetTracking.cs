...
namespace VSX.UniversalVehicleCombat 
{
	...
	public class HUDTargetTracking : MonoBehaviour
    {
        ...
        [Tooltip("A list of all the trackable types displayable on the HUD target tracking")]
        [SerializeField]
        private List<TrackableType> displayableTypes = new List<TrackableType>();
        ...
        // Called when script is first added to a gameobject
        private void Reset()
        {

            // Make all trackable types displayable by default
            TrackableType[] allTrackableTypes = (TrackableType[])System.Enum.GetValues(typeof(TrackableType));
            displayableTypes = new List<TrackableType>(allTrackableTypes);
		...
		void Start()
		{
			...
			{
				Debug.LogError("Add a RadarSceneManager component to the scene. Cannot link target tracking UI to ITracker component");
			}
		...
		void Visualize(ITrackable target, bool isSelectedTarget)
		{
			...
			usedWidgetParamsByType[typeIndex][thisWidgetIndex].labelFieldValue = target.TrackableLabel;
			usedWidgetParamsByType[typeIndex][thisWidgetIndex].showLabelField = widgetSettingsByType[typeIndex].showLabelField;


			// Widget value field

			usedWidgetParamsByType[typeIndex][thisWidgetIndex].valueFieldValue = HUDDistanceLookup.Instance.Lookup(distanceToTarget);
        ...
        void LateUpdate()
		{
			...
			for (int i = 0; i < targetsList.Count; ++i)
			{
                ...
                // Don't show if this trackable type is not in the displayableTypes list
                if (!displayableTypes.Contains(targetsList[i].TrackableType))
                {
                    continue;
                }
